...
//$fh = fopen($data_file, 'w') or die("can't open file");
    ...
    foreach ($tests as $test) {
      ...
      //$types = count_it('value','code',$dir, 2); die;
      $types = count_it('value','code',$dir); die;
...
function count_it($iati_element, $attribute, $dir, $hierarchy = FALSE) {
             ...
             if ($xml = simplexml_load_file($dir . $file)) {
                ...
                if(!xml_child_exists($xml, "//iati-organisation"))  { //exclude organisation files
                    ...
                    foreach ($elements as $element) {
                      if ($hierarchy && !((string)$element->attributes()->hierarchy == $hierarchy)) {
                        continue 1;
                      }
                      
                        $our_node = $element->{$iati_element};
                        if ($iati_element == "value") {
                        ...
                        if (isset($our_node) && count($our_node) > 0) { //something not quite right here
                          ...
                          foreach ($our_node as $transaction) {
                            ...
                            $transaction_types = $transaction->{'value'};
                            foreach ($transaction_types as $transaction_type) {
                              $no_transaction_types++;
                              //Gives a count of the total number of these elements
                              //$total_participating_orgs++;
                              //echo (int)$transaction_type;
                              if ((int)$transaction_type < 0) {
                                $activities_with++;
                                echo (int)$transaction_type . "," . (string)$element->{'iati-identifier'} . "," . $transaction_date . "," . $file . PHP_EOL;
                              }
                              //echo (int)$transaction_type;
                            }
