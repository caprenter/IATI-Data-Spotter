...
if (in_array($myinputs['group'],array_keys($available_groups))) {
    ...
    $results = check_last_updated_datetime_time ($dir,$url);

    print('<div id="main-content">
            <h4>Checking the @last-updated-datetime attribute</h4>');
            
      //Print out Files with no elements
      if ($results["files"] != NULL) {
        echo '<p class="cross">Some files need attention. Sort the table to find them</p>';
      } else {
          echo '<p class="tick">Seems as though all activities have a valid @last-updated-datetime.</p>';
      }
...
function check_last_updated_datetime_time ($dir,$url) {
    $rows = '';
    $files=array();
    $i=0;
                  ...
                  if(!xml_child_exists($xml, "//iati-organisation"))  { //exclude organisation files
                    foreach ($xml as $activity) {
                      if (count($activity->children()) > 0 ) { //skips registry record entries
                        ...
                        $generated  = $activity->attributes()->{'last-updated-datetime'};
